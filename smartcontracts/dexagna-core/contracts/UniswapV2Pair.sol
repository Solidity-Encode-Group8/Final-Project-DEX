...
contract GlobalBaseV1Fees {
    ...
    mapping(address => uint256) public feeBalancePerToken;
    ...
    function _safeTransfer(address token, address to, uint value) private {
        (bool success, bytes memory data) = token.call(abi.encodeWithSelector(SELECTOR, to, value)); //FIXME inaki, I do not understand SELECTOR ?
        require(success && (data.length == 0 || abi.decode(data, (bool))), 'UniswapV2: TRANSFER_FAILED');
    }

    // Allow the GlobalBaseV1Fees to transfer fees to users
    // Need to be call by the right contract/wallet
...
contract UniswapV2Pair is IUniswapV2Pair, UniswapV2ERC20 {
    ...
    function swap(uint amount0Out, uint amount1Out, address to, bytes calldata data) external lock {
        ...
        */
        address _token0 = token0;
        address _token1 = token1;
        if (amount0In > 0) _update0(token0, amount0In * 3 /1000); // accrue fees for token0 and move them out of pool (the fee is 0.3% (=3/1000) of the initial amount)
        if (amount1In > 0) _update0(token1, amount1In * 3 /1000); // accrue fees for token1 and move them out of pool
        balance0 = IERC20(_token0).balanceOf(address(this)); // since we removed tokens, we need to reconfirm balances, can also simply use previous balance - amountIn/ 10000, but doing balanceOf again as safety check
        balance1 = IERC20(_token1).balanceOf(address(this));
        // The curve : x*y>=k
        require( balance0.mul(balance1) >= uint(_reserve0).mul(_reserve1), 'K'); // BaseV1: K
        }
